...
function onUpdate(dc as Dc) as Void { 
        ...
        var icon = WatchUi.loadResource(Rez.Drawables.ProfileSettingsIcon);
        dc.drawBitmap(centerX - (icon.getWidth() / 2), (height * 0.25).toNumber(), icon);

        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.drawText(centerX, (height * 0.57).toNumber(), Graphics.FONT_MEDIUM, "Profile", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
